// Personal site: starfield background, scroll reveal, and on-demand embedding of Earth Chronicle.
...
// ---- embed Earth Chronicle on demand (it is a full WebGL app, so we load it only when asked)
const demo = document.getElementById('demo');
const launch = document.getElementById('demo-launch');
function launchDemo() {
  if (demo.classList.contains('live')) return;
  const iframe = document.createElement('iframe');
  iframe.src = '/earth/?embed=1';
  iframe.title = '地球纪元 · Earth Chronicle';
...
if (location.hash === '#demo') launchDemo();
